...
def get_victim_model_inference_pipeline(model_id):
    tokenizer = AutoTokenizer.from_pretrained(model_id)
    model = AutoModelForCausalLM.from_pretrained(
        model_id,
        torch_dtype=torch.bfloat16,
...
# ===================================================================
# demo_item_list = get_prompts("xxx.json")
...
#     first_score,first_disguised_prompt,first_victim_response = hypo_method(harmful_prompt,attacker_pipe,attacker_tokenizer,victim_pipe,victim_tokenizer,judgetype,10)
...
# with open("./markov_results/results_3_gpt4o_space.json", 'w') as file:
...
for item in tqdm(demo_item_list):
    ...
    temp_disguised_prompt = "xxx"
    temp_victim_response = "xxx" 
    first_score,first_disguised_prompt,first_victim_response = history_method(harmful_prompt,attacker_pipe,attacker_tokenizer,victim_pipe,victim_tokenizer,judgetype,10)
    temp_score = first_score
    temp_disguised_prompt = first_disguised_prompt
    temp_victim_response = first_victim_response   
    if temp_score == 1.0:
